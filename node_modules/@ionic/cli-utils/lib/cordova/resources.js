...
function flattenResourceJsonStructure() {
    return array_1.flattenArray(Object.keys(exports.RESOURCES).map(platform => (Object.keys(exports.RESOURCES[platform]).map(resType => (exports.RESOURCES[platform][resType]['images'].map((imgInfo) => ({
        platform,
        resType,
        name: imgInfo.name,
        width: imgInfo.width,
...
function getSourceImages(buildPlatforms, resourceTypes, resourceDir) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const srcImageDirContentList = yield Promise.all(srcDirList.map(srcImgDir => fs_1.readDir(srcImgDir.path)));
        const sourceImages = array_1.flattenArray(srcImageDirContentList.map((srcImageDirContents, i) => (srcImageDirContents
            .map((imgName) => {
 ...
 */
function provideDefaultResources(env, platform) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        if (yield fs_1.pathExists(destinationDir)) {
            ...
            yield fs_1.copyDirectory(sourceDir, destinationDir);
            env.tasks.end();
        }
    });
}
exports.provideDefaultResources = provideDefaultResources;
function ensureDefaultResources(env) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const os = yield Promise.resolve().then(function () { return require('os'); });
        const { tarXvfFromUrl } = yield Promise.resolve().then(function () { return require('../utils/archive'); });
        let recreateTmpDir = false;
        const tmpResourcesDir = path.resolve(os.tmpdir(), 'ionic-default-resources');
        try {
            const stat = yield fs_1.fsStat(tmpResourcesDir);
            if (new Date().getTime() - stat.ctime.getTime() > 604800000) {
                recreateTmpDir = true;
            }
        }
        catch (e) {
        ...
        if (recreateTmpDir) {
            ...
            yield tarXvfFromUrl(env, DEFAULT_RESOURCES_URL, tmpResourcesDir, { progress: (loaded, total) => {
                    task.progress(loaded, total);
                } });
            const resourcesDirFiles = yield Promise.all((yield fs_1.readDir(tmpResourcesDir)).map((f) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                const p = path.resolve(tmpResourcesDir, f);
                try {
                    const stat = yield fs_1.fsStat(p);
                    if (stat.isFile()) {
                        return p;
                    }
                }
            ...
            const pngImages = resourcesDirFiles.filter(p => typeof p === 'string' && path.extname(p) === '.png'); // TODO: typescript brokd
            for (let img of pngImages) {
                yield fs_1.cacheFileChecksum(img);
            }
            env.tasks.end();
        }
