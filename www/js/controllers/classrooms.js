angular.module('hdrApp').controller('ClassroomsController', function($scope, $rootScope, hdrFileSystem, $filter, $window,$state, $ionicLoading) {
    ...
    $scope.show = function() {
        $ionicLoading.show({
            template: '<ion-spinner icon="lines"></ion-spinner><br/><span dir="rtl">استخراج المعلومات...</span>'
        });
    };
    $scope.hide = function() {
        $ionicLoading.hide();
            ...
            ionic.Platform.ready(function() {
                ...
                hfs.getFileSystem('0').then(function(fs) {
                    hfs.isHdrDirectoryExist(fs).then(function(directory) {
                        ...
                        $scope.show();
                        hfs.readHdrFiles(directory).then(function(arrayofclassrooms) {
                            $rootScope.classrooms = arrayofclassrooms;
                            var arrlevel = [];
                            var i = 0;
                            angular.forEach($rootScope.classrooms, function(classroom) {
