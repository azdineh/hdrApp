...
.factory('hdrFileSystem', function($q,hdrKissm,$cordovaFile){
        ...
        HdrFileSystem.isHdrFile=function(fileentry){
            ...
            HdrFileSystem.getWorkbook(fileentry).then(function(workbook){ 
                var kissm=new hdrKissm();
                if(kissm.isMasarNotesCC(workbook)){
                    deferred.resolve(workbook);
                }else{
                    //alert("This Excel file is not conform to Masar file");
                    var msg= "يبدو أن ملف الإكسيل الجاري لا يتطابق مع بنية ملف مسار. المرجو التأكد من أن الملف يمثل وثيقة مسار الخاصة بنقط المراقبة المستمرة";
                    deferred.reject(msg);
                };
         ...
         */
        HdrFileSystem.readHdrFiles = function(directoryentry){
            //var hdrFiles=[];
            var classrooms=[];
            var deferred =$q.defer();
            ...
            directoryreader.readEntries(function(entries){
                if(entries.length>0){
                    ...
                    angular.forEach(entries, function(entry){
                        HdrFileSystem.isHdrFile(entry).then(function(workbook){
                            var kissmservice=new hdrKissm();
                            var kissm= kissmservice.getKissm(workbook);
                            classrooms.push(kissm);
                            if(classrooms.length===entries.length){
